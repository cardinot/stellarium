                                               ...
                                               const qsizetype bodyEndPos, const QString& locale, const QString& sectionName) const
{
	...
	for(const auto& entry : translations[locale])
	{
		...
		if(entry.comment == QString("Sky culture %1 section in markdown format").arg(sectionName.trimmed().toLower()))
			qWarning() << " *** BAD TRANSLATION ENTRY for section" << sectionName;
	}
	return text;
}
	...
	for(auto dictIt = translations.begin(); dictIt != translations.end(); ++dictIt)
	{
		...
		for(const auto& entry : dictIt.value())
		{
			...
			if(!entry.comment.isEmpty())
				po_message_set_comments(msg, entry.comment.toStdString().c_str());
			if(!entry.extractedComment.isEmpty())
				po_message_set_extracted_comments(msg, entry.extractedComment.toStdString().c_str());
